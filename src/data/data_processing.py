...
class DataAnalyzer:
    ...
    def data_brief(self, th_value_count=10, nrow=10):
        ...
        try:
            ...
            if cat_value_counts:
                cat_table = tabulate(cat_value_counts, 
    ...
    def analyze_missing_data(self):
        ...
        try:
            ...
            if not missing_counts.empty:
                ...
                missing_data = [[col, count, f"{pct:.2f}%"] 
                              for col, count, pct in zip(missing_counts.index, 
                                                       missing_counts.values, 
                                                       missing_percentages.values)]
                
                missing_table = tabulate(missing_data, 
                                       headers=["Column", "Missing Count", "Missing Percentage"], 
                                       tablefmt="grid")
    ...
    def preprocess_n_analyze_data(self):
        ...
        try:
            ...
            datetime_columns = ['transactionDateTime', 'currentExpDate', 
...
def main():
    ...
    try:
        ...
        summary_stats = {
            ...
            'processing_timestamp': pd.Timestamp.now().strftime('%Y-%m-%d %H:%M:%S'),
            'data_types_summary': {str(k): int(v) for k, v in analyzed_df.dtypes.value_counts().to_dict().items()},  # Convert keys and values
