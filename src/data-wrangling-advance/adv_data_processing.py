...
class TransactionAnalyzer:
    ...
    def identify_reversed_transactions(self):
        ...
        try:
            ...
            selected_columns = ['accountNumber', 'customerId', 'creditLimit', 
                              'transactionAmount', 'merchantName', 'cardLast4Digits',
            ...
            not_linked_sum = (self.df[self.df['transactionType'] == 'REVERSAL']['transactionAmount'].sum() - 
            ...
            self.transaction_stats.append(f"- Median Reversed Amount: ${reversed_df['transactionAmount'].median():,.2f}")
    ...
    def identify_multi_swipe(self, time_window=5):
        ...
        try:
            ...
            duplicated_cols = ['accountNumber', 'customerId', 'transactionAmount', 
                            'merchantName', 'cardLast4Digits']
            df_duplicated = df_purchase[df_purchase.duplicated(duplicated_cols, keep=False)].copy()  # Create a copy
            
            # Calculate time differences
            df_duplicated['timeDifference'] = df_duplicated.groupby(
                ['accountNumber', 'customerId', 'merchantName'])['transactionDateTime'].diff()
                # Identify multi-swipe transactions
            multi_swipe_df = df_duplicated[
                df_duplicated['timeDifference'] < pd.Timedelta(minutes=time_window)]
            ...
            self.transaction_stats.append(f"- Total Multi-Swipe Amount: ${multi_swipe_df['transactionAmount'].sum():,.2f}")
            self.transaction_stats.append(f"- Average Multi-Swipe Amount: ${multi_swipe_df['transactionAmount'].mean():,.2f}")
            self.transaction_stats.append(f"- Median Multi-Swipe Amount: ${multi_swipe_df['transactionAmount'].median():,.2f}")

            # Time difference analysis
            time_diff_stats = multi_swipe_df['timeDifference'].describe()
            self.transaction_stats.append("\nTime Difference Analysis:")
            self.transaction_stats.append(tabulate(
                time_diff_stats.to_frame(), 
                headers=['Time Difference Statistics'], 
                tablefmt="grid"
            ))
...
class FeatureProcessor:
    ...
    def remove_special_transactions(self, reversed_df, multi_swipe_df):
        ...
        try:
            ...
            merged_df = self.df.merge(reversed_df, 
                                    on=reversed_df.columns.to_list(), 
                                    how="left", 
                                    indicator=True)
            ...
            merged_df_ms = self.df.merge(multi_swipe_df[common_columns], 
                                    on=common_columns,
                                    how="left", 
                                    indicator=True)
    ...
    def handle_missing_values(self):
        ...
        try:
            ...
            if not initial_missing.empty:
                ...
                self.processing_text.append(tabulate(missing_stats, 
    ...
    def derive_features(self):
        ...
        try:
            ...
            columns_to_drop = ['accountNumber', 'customerId', 'cardCVV', 'enteredCVV', 
                             'cardLast4Digits', 'transactionDateTime', 'currentExpDate', 
                             'accountOpenDate', 'dateOfLastAddressChange', 'availableMoney']
...
class FeatureEncoder:
    ...
    def encode_features(self):
        ...
        try:
            ...
            self.encoding_text.append(tabulate(encoding_stats, 
                                            headers=['Feature', 'Method', 'Categories'],
                                            tablefmt="grid"))
        ...
        try:
            ...
            for col in columns_to_normalize:
                ...
                scaling_stats.append([
                    col,
                    f"{orig_stats['min']:.2f} to {orig_stats['max']:.2f}",
                    "0.0 to 1.0"
                ])
            ...
            self.scaling_text.append(tabulate(scaling_stats,
                                           headers=['Feature', 'Original Range', 'Scaled Range'],
                                           tablefmt="grid"))
...
class DataVisualizer:
    ...
    def plot_transaction_distributions(self, reversed_df, multi_swipe_df):
        try:
            ...
            sns.histplot(data=reversed_df, x='transactionAmount', 
                        kde=True, label='Reversed Transactions')
            sns.histplot(data=multi_swipe_df, x='transactionAmount', 
                        kde=True, label='Multi Swipe')
            ...
            logger.save_logs(f"Error in plotting distributions: {str(e)}", 
                           log_level='error')
            raise
    ...
    try:
        ...
        summary_stats = {
            ...
            'removed_transactions': {
                'reversed': int(len(reversed_df)),
                'multi_swipe': int(len(multi_swipe_df))
            }
        }
